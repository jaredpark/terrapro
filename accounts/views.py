...
from accounts.models import Property, PropertyReport

def home(request):
    return(HttpResponseRedirect(reverse(profile_detail, args=[request.user.username])))

def property_detail(request, username=None, property_pk=None,
                   template_name='accounts/property_home.html',
                   extra_context=None):
    if username is None:
    ...
    property_list = profile_obj.Properties.all()
    context['property_list'] = property_list
    # property_info = profile_obj.Properties.filter(pk=property_pk)
    property_info = Property.objects.filter(organization__user__username=username, pk=property_pk)
    context['property_info'] = property_info

    report_list = PropertyReport.objects.filter(report_property__organization__user__username=username, report_property__pk=property_pk).order_by('-date')
